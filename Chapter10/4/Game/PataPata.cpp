...
void PataPata::WaveForwardUpdate()
{
	...
	circle_.pos = pos_;
	if (fabsf(pos_.y - player_->GetPos().y)<=200) {
		ChangeWait();
	}
}

void PataPata::WaveForwardUpdate2()
{
	pos_ += vel_;
	pos_.x += sinf(((float)frame_ / 20.0f) * sign_);
	circle_.pos = pos_;
...
{
	vel_ = { 0,move_speed };
	update_ = &PataPata::WaveForwardUpdate2;
}
void PataPata::ChangeBack() {
	update_ = &PataPata::WaveBackUpdate;
	if ((320 - pos_.x) != 0.0f) {
		vel_.x = (320 - pos_.x) / abs(320 - pos_.x);
	};
