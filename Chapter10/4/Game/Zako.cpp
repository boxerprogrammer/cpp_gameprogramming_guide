...
void Zako::ForwardUpdate()
{
	...
	pos_ += vel_;
	circle_.pos = pos_;
	if (fabsf(pos_.y - player_->GetPos().y) <= 200.0f) {
		DoMoveNear();
	}
	
}

void Zako::MoveNearUpdate()
{
...
void Zako::ChangeBack()
{
	...
	auto vec = (player_->GetPos() - pos_);
	vec.x *= 0.1f;
	if (vec.Length() > 0.0f) {
		vel_ = vec.Normalized() * back_move_speed;
		return;
	}
...
void Zako::DoMoveNear()
{
	const auto& ppos = player_->GetPos();
	vel_.x = (ppos.x - pos_.x)/fabsf(ppos.x - pos_.x);
	vel_.y = 1.0f;
	vel_ = vel_.Normalized() * back_move_speed;
	update_ = &Zako::MoveNearUpdate;
}
