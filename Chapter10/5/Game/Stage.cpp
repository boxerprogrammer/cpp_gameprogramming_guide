...
void Stage::Load(int stageNo)
{
	...
	int idx = 0;
	//ここで実際のそれぞれのレイヤーデータを読み込む
	for (auto& raw : rawData) {
		//それぞれのレイヤーの処理
		raw.resize(header.width * header.height);//確保
		data_[idx].resize(raw.size());
		//ここでまず、データ本体を一気に読み込みます
		FileRead_read(raw.data(), raw.size() * sizeof(uint8_t), handle);//読み込み
		...
		for (int y = 0; y < header.height; ++y) {
			...
			std::copy_n(&raw[y * header.width],//コピー元の先頭アドレス
				header.width,//コピーする個数
				&data_[idx][revY * header.width]);//コピー先の先頭アドレス
		}
		++idx;
	}
...
uint8_t Stage::GetData(int xidx, int yidx,int layerNo)
{
	return data_[layerNo][yidx*dataSize_.w+xidx];
}

const std::vector<std::vector<uint8_t>>& Stage::GetAllData() const
{
	return data_;
}
